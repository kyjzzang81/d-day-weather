...
현재 코드 기준으로는 미설정 상태에서도 기본 선호 활동을 반환한다.

```text
localStorage key: ggg.activityPreferences.v1
default: urban, photo, cafe
```

따라서 현재 앱에서는 첫 진입 시 TODAY 활동 추천 섹션에 기본 3개 활동이 보인다.  
미설정 CTA를 반드시 노출하려면 `activityPreferenceStore`에서 저장값 없음과 기본값을 분리해야 한다.
...
아이콘은 `assets/activity-icons/optimized/activity-{code}.webp`를 사용한다.
...
```json
{
  "selected_activity_count": 3,
  "selected_activities": ["urban", "photo", "cafe"],
  "default_theme_overlay": null,
  "activity_preference_set": true,
...
- 위치 권한 안내
- 지역 직접 선택 대체 경로
- TODAY 선호 활동 기본값/설정 화면
- 선호 활동 설정 화면
- 선호 활동 최대 3개 선택
- localStorage 기반 비로그인 선호 활동 저장
- 사이드바 `선호 활동 변경`
