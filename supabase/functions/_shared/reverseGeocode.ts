...
export async function reverseGeocode(lat: number, lon: number): Promise<string | null> {
  ...
  const response = await fetch(`https://nominatim.openstreetmap.org/reverse?${params.toString()}`, {
    headers: {
      "User-Agent": "ggg-mvp/1.0 (today-location; contact@ggg.app)"
    }
  });
  ...
  const label = formatKoreanDisplayLocation(body.address, body.display_name);
  return label || null;
}

export function formatKoreanDisplayLocation(address?: NominatimAddress, displayName?: string): string | null {
  if (!address) {
